...
const AllBanner = () => {
  ...
  const fetchBanners = async () => {
    setLoading(true);
    try {
      const response = await axios.get("http://localhost:4255/api/v1/get-banner");
  ...
  const handleDelete = async (id, publicId) => {
    try {
      await axios.delete(`http://localhost:4255/api/v1/delete-banner/${id}`, {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:4255/api/v1/update-banner/${selectedBanner._id}`,
        formData,
