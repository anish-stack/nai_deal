...
const AllBanner = () => {
  ...
  const fetchBanners = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://www.api.naideal.com/api/v1/get-banner");
  ...
  const handleDelete = async (id, publicId) => {
    try {
      await axios.delete(`https://www.api.naideal.com/api/v1/delete-banner/${id}`, {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.post(
        `https://www.api.naideal.com/api/v1/update-banner/${selectedBanner._id}`,
        formData,
